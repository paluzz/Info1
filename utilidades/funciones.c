...
// Transforma string a flotante
float transformarAFloat(char *str)
{
    float num, entero = 0, decimal = 0, exp[] = {1, 10, 100, 1000, 10000}, expf[] = {0.1, 0.01, 0.001, 0.0001, 0.00001};
    int i, j, l = strlen(str), flotante = 0, contador = 0;

    // comprobamos si tiene punto flotante
    for (i = 0; i < l; i++)
    {
        if (*(str + l - 1 - i) == '.' || *(str + l - 1 - i) == ',')
        {
            flotante = 1;
            break;
        }
        // buscamos cuantos decimales hay
        contador++;
    }
    if (flotante == 0)
    {
        // solo tiene parte entera
        for (i = 0; i < l; i++)
        {
            if (*(str + l - 1 - i) == '.' || *(str + l - 1 - i) == ',')
            {
            ...
            entero += (*(str + l - 1 - i) - '0') * exp[i];
        }
        num = entero;
    }
    else
    {
        // Tiene parte entera y decimal
        // parte decimal
        for (i = 0; i < l; i++, contador--)
        {
            if (*(str + (l - 1) - i) == '.' || *(str + (l - 1) - i) == ',')
            {
                break;
            }
            decimal += (*(str + (l - 1) - i) - '0') * expf[contador - 1];
        }
        // parte entera
        for (i += 1, j = 0; i < l; i++, j++)
        {
            entero += (*(str + l - 1 - i) - '0') * exp[j];
        }
        num = entero + decimal;
    }
...
// recibe string con operaciones basicas
float operacionesBasicas(char *p)
{
    int i, j, l = strlen(p), flag = 1;
    float signo1 = 1, signo2 = 1, op1, op2, resultado;
    char operador;
    char *aux = (char *)malloc(sizeof(char) * l);
    char *str1 = (char *)malloc(sizeof(char) * (l / 2));
    char *str2 = (char *)malloc(sizeof(char) * (l / 2));
    if (aux == NULL || str1 == NULL || str2 == NULL)
    {
        return (-1);
    }
    ...
    i = 0, j = 0;
    // copiamos la primera parte del string
    while (flag == 1)
    {
        // comprobar signo del primer caracter
        if (aux[0] == '-')
        {
            signo1 = -1;
            i++;
            aux[0] = 'f'; // evitamos que vuelva a entrar
        }
        // guardamos operador y salimos
    ...
    // pasamos la primera parte a float
    op1 = signo1 * transformarAFloat(str1);

    j = 0, i++;
    // copiamos la segunda parte del string
    while (aux[i] != '\0')
    {
        // comprobar signo del primer caracter
        if (aux[i] == '-')
    ...
    str2[j] = '\0';
    // pasamos la segunda parte a float
    op2 = signo2 * transformarAFloat(str2);
    ...
    // Una vez hecha toda la magia ahora simplemente hacer la operacion
    switch (operador)
    {
    case '+':
        resultado = op1 + op2;
